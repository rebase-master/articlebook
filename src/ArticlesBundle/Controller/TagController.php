...
class TagController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $tags = $em->getRepository('ArticlesBundle:Tag')->findAll();
    ...
    {
        $tag = new Tag();
        $form = $this->createForm('ArticlesBundle\Form\TagType', $tag);
    ...
    {
        $deleteForm = $this->createDeleteForm($tag);
        $editForm = $this->createForm('ArticlesBundle\Form\TagType', $tag);
        $editForm->handleRequest($request);

        if ($editForm->isSubmitted() && $editForm->isValid()) {
            $this->getDoctrine()->getManager()->flush();

            return $this->redirectToRoute('admin_tags_edit', array('id' => $tag->getId()));
        }
    ...
    private function createDeleteForm(Tag $tag)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('admin_tags_delete', array('id' => $tag->getId())))
            ->setMethod('DELETE')
            ->getForm()
        ;
    }
